...
router.patch('/recharges/:id/approve', authenticateAdmin, async (req, res) => {
  ...
  try {
    ...
    const userId = rechargeRows[0].user_id;
    const amountNum = parseFloat(rechargeRows[0].amount);
    ...
    if (userRows.length) {
      ...
      const bonuses = [
        ...
        { id: level3_id, percent: 0.01, type: 'level3' },
      ];
      ...
      for (const bonus of bonuses) {
        ...
        await connection.query(
          ...
          [bonus.id, userId, bonusAmount, 'bonus']
        );
